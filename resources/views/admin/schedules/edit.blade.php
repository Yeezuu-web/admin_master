    ...
    <div class="card-body">
        <form id="form">
            ...
            <div class="form-group col-md-12">
                ...
                <input class="form-control" type="text" name="note" id="note" value="{{ old('note', $schedule->note) }}">
            ...
            <div class="form-group col-md-6">
                ...
                <select name="day_id" class="form-control">
                    @foreach ($days as $day)
                    <option value="{{$day->id}}" {{ $day->id == old('day_id', $schedule->day->id) ? 'selected' : '' }} >{{ $day->day }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-12">
                ...
                <select multiple name="file_id" id="bootstrap4_duallistbox">
                    ...
                    <option value="{{$file->id}}" {{ (in_array($file->id, old('file_id', [])) || $schedule->files->contains($file->id)) ? 'selected' : '' }}>ID: {{ $file->content_id }} || Title: {{ $file->title_of_content }}</option>
                    @endforeach
                </select>
            </div>
